...
The image installs the project with `uv sync --frozen`, so builds are reproducible with `uv.lock`. On every push to `master`, GitHub Actions also publishes the image to GHCR at `ghcr.io/<owner>/<repo>`. You can pull it via:
